...
def get_csv_header(algorithms=["FH04", "CC"]):
    header = [
        "Name",
        "|P|"
    ]
...
def main():
    ...
    for i in tqdm(range(n_scenes), desc=desc, disable=verbose):
        ...
        unions, probs = predict(graph_dict=graph_dict, dec_b=args.t)

        part_fh04 = partition_from_probs(graph_dict=graph_dict, sim_probs=probs, k=args.k, P=P, sp_idxs=sp_idxs)
        part_cc = partition(graph_dict=graph_dict, unions=unions, P=P, sp_idxs=sp_idxs, half=False)
        ...
        csv_data.append([file_gt, P.shape[0], ooa_fh04, size_fh04, ooa_cc, size_cc])
